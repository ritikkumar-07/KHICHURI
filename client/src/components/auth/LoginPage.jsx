...
export function LoginPage({ initialMode = null, onSuccess, onCancel }) {
  ...
  return <section className="auth-card"><span className="section-kicker">{mode === 'admin' ? 'AUTHORIZED HOSPITAL ACCESS' : 'PERSONAL HEALTH ACCESS'}</span><h1>{mode === 'admin' ? 'Hospital Admin' : creating ? 'Create your account' : 'Welcome back'}</h1><form onSubmit={submit}><label>Email<input type="email" autoComplete="email" required value={email} onChange={event => setEmail(event.target.value)} /></label><label>Password<input type="password" autoComplete={creating ? 'new-password' : 'current-password'} minLength="6" required value={password} onChange={event => setPassword(event.target.value)} /></label>{message && <div className="auth-message" role="alert">{message}</div>}<button className="button primary" disabled={busy}>{busy ? 'Please wait…' : mode === 'admin' ? 'Admin Sign In' : creating ? 'Create Account' : 'Sign In'}</button></form>{mode === 'user' && <button className="text-button" onClick={() => { setCreating(!creating); setMessage(''); }}>{creating ? 'Already have an account? Sign In' : "Don't have an account? Create Account"}</button>}<button className="text-button" onClick={() => { setMode(null); setCreating(false); setMessage(''); }}>Back to login options</button></section>;
}
